...
| Предложение эксперимента | Второй versioned proposal task по frozen ExplanationSet/tree; strict schema, host-owned synthetic recipe, atomic exploratory protocol/tree node/application и сохранение invalid/abstained/stale ответов. | Реальный model call для этого task, независимая научная оценка дизайна, автоматический выбор/исполнение и перенос на второй domain pack. |
...
**Инкремент 17 сентября:** ResearchQuestion/ExplanationSet revisions с optimistic head checks и объяснением исключений, `preregister_for_set`, immutable planning binding, reviewer conflicts всей ancestry и study consistency command context. [ADR 0004](decisions/0004-planning-lineage.md) отделяет эти declarations от реального исполнения ограничений и научной оценки.

M1 в целом ещё открыт: остальные публичные entity schemas, полные resource/domain/data declarations вопроса, HypothesisVersion lineage, общий schema migration и planned-versus-observed domain validation не реализованы. Ниже сохранены полные критерии этапа.
...
**Инкремент 23 сентября:** [model proposals](decisions/0007-model-proposals.md) реализовали первый provider task: Codex CLI response → verified proposal → atomic hypotheses/root ExplanationSet. Сохраняются budget, prompt/schema/context, original outputs, usage и failure history. Этот срез не содержал экспериментального предложения; literature retrieval, model tournament judging и качество гипотез ещё не реализованы/не оценены.

**Инкремент 24 сентября, локально реализован:** [предложение эксперимента](decisions/0008-experiment-proposals.md) получает frozen ExplanationSet и Search tree, ограничивает модель одним synthetic recipe и применяет допустимый ответ как exploratory protocol плюс tree node в одной транзакции. `agent recipe --input` сохраняет host-owned world/seeds/environment binding без вызова модели; опубликована [JSON schema ответа](../schemas/experiment-proposal-v1.schema.json). Versioned receipts, replay/recovery, stale state и CLI recipe проверены целевыми fixture/интеграционными тестами; полный suite — 406 тестов, 4 Windows symlink skips. Один реальный model call для нового task успешно применён, но качество теста не оценено научным review. [CI кода реализации](https://github.com/AdamCage/EpistemeOS/actions/runs/36003409069) прошёл во всех четырёх конфигурациях; выбор/запуск эксперимента, evidence и scientific review не выполнены.
